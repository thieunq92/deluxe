    ...
    $scope.save = function (control) {
        if ($("#aspnetForm").valid()) {
            for (var i = 0; i<$rootScope.campaign.GoldenDays.length;i++){
                ...
                $rootScope.campaign.GoldenDays[i].Policy = !$rootScope.campaign.GoldenDays[i].Policy ? $rootScope.campaign.GoldenDays[i].Policy.replace(new RegExp('\n', 'g'),'<br/>') : '';
            }
            $http({
                method: 'POST',
                url: 'WebMethod/GoldenDayCreateCampaignWebService.asmx/GoldenDaySaveOrUpdate',
                data: { campaign : $rootScope.campaign },
            }).then(function (response) {
