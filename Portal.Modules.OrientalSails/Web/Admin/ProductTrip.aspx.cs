...
{
    public partial class ProductTrip : SailsAgencyAdminBase
    {
        #region -- Private Member --

        private readonly ILog _logger = LogManager.GetLogger(typeof(ProductTrip));
            ...
            if (item != null)
            {
                ...
                using (HyperLink hyperLink_Name = e.Item.FindControl("hyperLink_Name") as HyperLink)
                {
                    if (hyperLink_Name != null)
                    {
                        ...
                        hyperLink_Name.NavigateUrl = "javascript:;";
                        var url = string.Format(
                            "ProductTripDetail.aspx?NodeId={0}&SectionId={1}&TripId={2}", Node.Id, Section.Id, item.Id);
                        hyperLink_Name.Attributes.Add("onclick", "openPopup('" + url + "', 'Trip', 600, 800)");
                    }
                }
                ...
                using (HyperLink hyperLinkEdit = e.Item.FindControl("hyperLinkEdit") as HyperLink)
                {
                    if (hyperLinkEdit != null)
                    {
                        hyperLinkEdit.NavigateUrl =
                            string.Format("ProductTripDetail.aspx?NodeId={0}&SectionId={1}&TripId={2}",
